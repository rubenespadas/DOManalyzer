...
exports.Window = function(html, url, config, cookie, mimetype, callback) {
    ...
    try {
        ...
        window.XMLHttpDownload = function(xml, url) {
            ...
            window.results.activity = utils.addString(window.results.activity, msg);
            window.results.objects = utils.addString(window.results.objects, utils.absoluteUrl(window.results.url, url));
            loggers.log(msg, url);
        };
        ...
        Object.defineProperty(window.navigator, 'plataform', {
                get: function() {
                    var msg = 'window.navigator.plataform';
                    window.results.activity = utils.addString(window.results.activity, msg);
                    loggers.log(msg, true);
                    return config.plataform;
                }
            }
        );
                ...
                if(element){
                    ...
                    element.setAttribute = function(attr, value) {
                        if(attr.toLowerCase()==='src') {
                            ...
                            window.results.activity = utils.addString(window.results.activity, 'Hidden element');
